...
template <typename Item, typename K = typename Item::KeyType, typename H = Hash<K>, size_t LowerBound = 50> class HashTable {
...
public:
	...
	HashTable(size_t capacity = LowerBound + LowerBound / 2 + 1) : _capacity(capacity) {
	...
	// Вставка
	typename Item::ValueType& insert( const Item& t, bool doAutorehash = true ) {
		if ( doAutorehash ) autorehash();
		K k = t.key();
		size_t i = hash_(k);
	...
	// Вставка списка.
	void insert(std::initializer_list<Item> il) {
		insert(il.begin(), il.end());
	}
	
	// Удаление.
	bool remove(const K& k, bool doAutorehash = true) {
		size_t i = hash_(k);
		Link p = _heads[i];
		Link n = p;
	...
	void remove(It begin, It end) {
		for( It i = begin; i != end; ++i ) remove(*i, false);
		autorehash();
	}
	// Удаление списка.
	void remove(std::initializer_list<K> il) {
		remove(il.begin(), il.end());
	}
	...
	void autorehash() {
		// Если таблица заполнена > 3/4 или < 1/4 рехэшируем.
		if (_size > LowerBound && (_capacity <= _size + (_size >> 1) || (_capacity >> 2) > _size)) {
			rehash((_size << 1) + 1);
		}
		assert(_capacity >= LowerBound * 2);
	}
	
	// Резервируем такое количество места, чтобы без рехэширования влезло capacity элементов.
	void reserve(size_t capacity) {
		if (capacity < LowerBound * 3 / 2) {
			assert(_capacity >= LowerBound * 2);
			return;
		}
		size_t topRange = capacity + (capacity << 2);
		if (_capacity < topRange || (_capacity >> 2) > topRange) {
			rehash(topRange + 1);
		}
	}
